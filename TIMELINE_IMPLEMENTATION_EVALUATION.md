...
使用者提供的示意圖可整理成三個相同結構、不同 scope 的工作區畫面：
...
工作區 2：主時間軸 > 某個大箱
  ├── Breadcrumb：主時間軸 > 某個大箱
  ├── 上方時間軸總覽：顯示該大箱範圍內的中箱
  └── 下方節點面板：Tick 數、關聯章節、加入關聯

工作區 3：主時間軸 > 某個大箱 > 某個中箱 > 某個小箱
  ├── Breadcrumb：完整父子路徑
  ├── 上方時間軸總覽：顯示目前小箱 scope，選取小箱使用藍色高亮
  └── 下方節點面板：目前小箱的 Tick 數與關聯資料
```
...
- 左側固定的軌道名稱欄，例如「時間軸 1」「時間軸 2」「自訂名稱」。
- 右側共享水平 Tick 尺。
- 每個元素前後提供控制元件調整頭尾Tick。
- 目前 scope 下的元素卡片。
- 目前選取元素以藍色高亮；非選取元素維持中性灰色。
- 水平捲軸與可拖曳滑桿，用來瀏覽或縮放目前 Tick 範圍。
- 垂直軌道數增加時，時間軸區可垂直捲動，但軌道名稱欄需和內容同步。

顯示規則：

1. 根 scope 顯示大箱。
2. 進入大箱 scope 後，顯示該大箱的中箱；不把整個專案的其他大箱混入可編輯區。
3. 進入中箱 scope 後，顯示該中箱的小箱。
4. 父層名稱由 Breadcrumb 提供，子層內容由上方時間軸提供；不能只靠顏色判斷目前所在層級。
5. 目前選取的節點只代表 selection，不代表它被固定移到某個軌道；拖曳軌道仍可改變 `trackUUID`。
...
選取大箱、中箱或小箱後，顯示同一個可依層級重用的 `TimelineElementInspector`：
...
- `Tick 數` 對應 `durationTicks`，不可輸入小於 1 的值。
- 頭端控制修改 `startTick`；尾端控制修改 `endTick`/`durationTicks`。
- 章節按鈕代表目前節點或其葉節點的關聯章節；按 `+` 開啟章節選擇器。
- 若目前節點沒有可直接關聯的 `sceneUUID`，章節關聯可以作用於其葉節點，或顯示「請先選取小箱/場景」。產品需在 Phase 0 決定是否允許容器節點直接關聯章節。
- 任何端點修改都要經過父子 invariant、後續元素衝突與自動分軌流程，不可只更新面板數字。

### 16.3 三種畫面的互動對照

| 使用者操作 | 根時間軸 | 大箱 scope | 中箱 scope/小箱 scope |
|---|---|---|---|
| 點擊元素 | 選取大箱 | 選取中箱 | 選取小箱或葉節點 |
| 再次點擊元素 | 進入該大箱 | 進入該中箱 | 若有子元素則進入下一層 |
| Breadcrumb | 顯示主時間軸 | 可回主時間軸 | 可回大箱或主時間軸 |
| 下方面板 | 顯示大箱資料 | 顯示中箱資料 | 顯示小箱資料 |
| 頭/尾 Tick 控制 | 調整大箱並檢查中箱 | 調整中箱並檢查小箱 | 調整小箱並向父層延展 |
| 拖曳元素 | 移動大箱子樹 | 移動中箱子樹 | 移動小箱子樹 |

單擊應負責 selection，雙擊或明確的「進入」按鈕負責 scope navigation，避免使用者只是想查看下方屬性卻意外離開目前畫面。若產品決定單擊直接進入，必須讓節點卡片上的 selection/inspect 操作仍可獨立觸發。
...
```text
timelineScopeProvider       → 目前正在瀏覽哪個 parentPlacementUUID
timelineSelectionProvider   → 目前選取哪個 placementUUID
```

兩者不可合併成一個 ID，因為使用者可以在大箱 scope 中選取某個中箱，也可以暫時清除選取但仍留在大箱 scope。

當 scope 改變時：

1. 先保存上一個 scope 的水平 scroll anchor。
2. 以 `parentPlacementUUID` 查詢下一層元素。
3. 清除不屬於新 scope 的 selection，或把 selection 指向新 scope 的對應節點。
4. 更新 Breadcrumb。
5. 將目前 scope 的最早/最晚 Tick 傳給尺與水平捲動控制器。

當節點刪除時：

- 若刪除的是 selection，先清除下方節點面板。
- 若刪除的是目前 scope，回到最近仍存在的祖先。
- 若刪除的是 Breadcrumb 中間祖先，回到它的最近仍存在祖先，且顯示一次性提示。
...
- 灰色：未選取、可編輯元素。
- 藍色：目前選取 scope 或 selection。
- 淺藍色：目前 scope 的有效背景範圍。
- 紅色/橙色：端點調整後會造成同軌衝突或需要自動分軌。
- 虛線：父層有效範圍被子元素延展，或尚未完成 Tick/軌道配置。

所有自動行為都要在操作後給予可讀回饋，例如：

```text
中箱已向後延展 3 Tick；後續 2 個元素已移至「時間軸 3」。
```

這比只改變灰色區塊或軌道位置更容易讓使用者理解系統為什麼改動版面。
